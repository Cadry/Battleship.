...
public class Computer {
  ...
  public void getMove(Player p1) {
    ...
    boolean straightBarrage = false;
    
    if (p1.boardState == 0) { // Previous random
      int[] boardChoice;
    ...
    else if (p1.boardState == -1) { //Weight Random
    ...
    //Temporary copy to ensure not out of bounds
      int temp[] = new int[2];
      temp[1] = p1.coord[1];      //X? Unsure
      temp[0] = p1.coord[0] + 1;  //Y?
      if (temp [0] < p1.coord.length) {
        coord[1] = p1.coord[1];
        coord[0] = p1.coord[0] + 1;
        System.out.println("Computer fires at: " + coord[1] + ", " + coord[0] + ".");
      }
      else {
      ...
      int temp[] = new int[2];
      temp[1] = p1.coord[1] + 1;      //X
      temp[0] = p1.coord[0];          //Y
      if (temp [1] < p1.coord.length) {
        coord[1] = p1.coord[1] + 1;
        coord[0] = p1.coord[0];
        System.out.println("Computer fires at: " + coord[1] + ", " + coord[0] + ".");
      }
      else {
